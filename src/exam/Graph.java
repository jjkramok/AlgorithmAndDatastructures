...
public class Graph {
    ...
    public Graph(String vert[], int matrix[][]) {
        int vertIndex = 0;
        Vertex[] matrixVertices = new Vertex[matrix.length];

        //Create an array with all labelled vertices
        for (int[] column : matrix) {
            matrixVertices[vertIndex] =
                    new Vertex(vert[vertIndex]);
            vertIndex++;
        }

        //Fill adjacency list
        vertIndex = 0;
        for (int[] adjList : matrix) {
            for (int i = 0; i < adjList.length; i++) {
                if (adjList[i] == 1) {
                    Edge newEdge = new Edge(matrixVertices[i], 0);
                    matrixVertices[vertIndex].adj.add(newEdge);
                }
            }
            vertIndex++;
        }
        ...
        vertices = new HashSet<Vertex>();
        vertices.addAll(Arrays.asList(matrixVertices));
        /*for (Vertex v : matrixVertices) {
            vertices.add(v);
        }*/
    }
